package week9;
//code inspired from https://sites.google.com/site/indy256/algo/preflow and geeksforgeeks
...
public class Interstellar2 {
	public static void main(String[] args) throws Exception{
		...
		for (int tc = 1; tc <= t; tc++) {
			...
			ShrtPth st = new ShrtPth(N,n,flow);
...
class ShrtPth
{
	...
	public ShrtPth(int v,int nodes,PushRelabel flow){
		this.V = v;
		this.flow = flow;
		this.maxNodes = nodes;
	}
	...
	void dijkstra(double graph[][], int src)
	{
		...
		if(dist[V-1]==Double.MAX_VALUE){//no path
			...
			int allMax=flow.maxFlow(0, V*maxNodes);
			if(allMax==0){
				System.out.println("impossible");
			}
			else
