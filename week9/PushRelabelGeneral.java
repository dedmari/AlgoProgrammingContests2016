...
public class PushRelabelGeneral {
	...
	public static void main(String[] args) {
		int[][] capacity = { { 0, 5, 11, 2, 0}, { 5, 0, 0,6, 10 }, { 11, 0, 0,7,0 },{2,6,7,0,10},{0,10,0,10,0} };
		int n = capacity.length;
		PushRelabelGeneral flow = new PushRelabelGeneral();
		flow.init(n);
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				if (capacity[i][j] != 0)
					flow.addEdge(i, j, capacity[i][j]);
		System.out.println(flow.maxFlow(0, 1));
	}
}
